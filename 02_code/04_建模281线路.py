...
def show_pid(x_values, inv_y ,inv_yhat,title):
    fig = plt.figure()
    ax1 =fig.add_subplot(111)#1x1网格，第一个子图
    ax1.set_title(title) #设置标题 
    plt.xlabel('时间')  #设置X轴标签  
    plt.ylabel('乘客数量') #设置Y轴标签  
    plt.plot(list(x_values), list(inv_y),c='r') #画散点图 ：原始
    plt.plot(list(x_values), list(inv_yhat),c='b') #画散点图 ：预测
    plt.legend(["原始","预测"])#图例
    plt.show()  
    
#=============================================================================
#1. 读取数据
data = pd.read_csv("../11_data_result/ft_281.csv")
...
#2.2 划分训练集和测试集
data_281_train = data[data['date_hour'] <  2014122500]
data_281_test  = data[data['date_hour'] >= 2014122500]

#2.3 数据规范化化：z-score规范化
scaler = preprocessing.StandardScaler().fit(data[feature_list])

train_X = scaler.transform(data_281_train[feature_list])
train_Y = data_281_train['label'].values
...
#画出拟合曲线
show_pid(data_281_train.index, train_Y ,result_list, "训练集")

#===========================================================================
#测试集
data_281_test = data_281_test.sort_values(by="date_hour" , ascending=True)
data_281_test.reset_index(drop = True, inplace = True)

test_X = scaler.transform(data_281_test[feature_list])
test_Y = data_281_test['label'].values
...
#画出拟合曲线
show_pid(data_281_test.index, test_Y ,result_list, "测试集")


#保存模型
joblib.dump(linear_model, '../03_model/liner_281.model')#多元线性回归模型
joblib.dump(scaler, '../03_model/scalar_281.model')#数据标准化模型
